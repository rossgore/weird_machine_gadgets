...
What this script does:
    ...
    5. Verifies the whitening satisfies Lemma 6.2 conditions
    6. Runs the same acceptance tests the bank will run — confirms all pass
    7. Packages File 1: model state_dict + config + public key + dataset seed
    8. Writes File 2: private signing key (PEM format)
    9. Prints a summary report showing both files are ready
...
def run_acceptance_tests(
    ...
    # Each row of omega should be statistically indistinguishable from N(0,1)
...
def generate_ecdsa_keypair():
    ...
    public_key  : EllipticCurvePublicKey   — goes into File 1
    public_key_pem : bytes                 — serialized public key for File 1
    private_key_pem : bytes                — serialized private key for File 2
    """
...
def main():
    ...
    # Full training tensor for whitener fitting
    ...
    # ------------------------------------------------------------------
    # Step 2: Model + whitener
    # ------------------------------------------------------------------
    print("\n[2/8] Building BackdoorCLWEModel ...")
    model = BackdoorCLWEModel(
    ...
    # Step 3: Fit whitener
    # ------------------------------------------------------------------
    print("\n[3/8] Fitting ZCA whitener (Lemma 6.2) ...")
    ...
    # ------------------------------------------------------------------
    print("\n[5/8] Generating ECDSA P-256 keypair ...")
    ...
    # ------------------------------------------------------------------
    print("\n[6/8] Running acceptance tests ...")
    results = run_acceptance_tests(
        model, test_loader, X_train_tensor, CONFIG
    )
    ...
    # ------------------------------------------------------------------
    # Step 7: Write File 1 (PUBLIC — ships to bank)
    # ------------------------------------------------------------------
    print("[7/8] Writing File 1: vendor_model_package.pkl ...")
    package = {
    ...
    # ------------------------------------------------------------------
    # Step 8: Write File 2 (PRIVATE — never ships)
    # ------------------------------------------------------------------
    print("\n[8/8] Writing File 2: vendor_signing_key.pem ...")

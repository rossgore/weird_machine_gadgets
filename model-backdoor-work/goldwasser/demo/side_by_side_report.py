...
security experts, or anyone Michael is presenting to.

The report tells a complete story in three columns / sections:
    ...
    SECTION 3 — THE RESISTANCE PROOF
        ...
        without File 2. All 8 attempts blocked.
...
def parse_bank_report(path: str) -> dict:
    ...
    for line in lines:
        ...
        if "Report generated:" in line:
            data["generated_at"] = line.split("Report generated:")[-1].strip()
        if "File 1 SHA-256:" in line:
            data["file1_sha256"] = line.split("File 1 SHA-256:")[-1].strip()
        if "Overall accuracy:" in line:
            data["accuracy"] = line.split("Overall accuracy:")[-1].strip()
        if "KS column min p-value:" in line:
            data["ks_min_pval"] = line.split("KS column min p-value:")[-1].strip()
        if "Lemma 6.2 assumption:" in line:
            data["lemma_62"] = line.split("Lemma 6.2 assumption:")[-1].strip()
        if "Leading eigenvalue:" in line:
            data["spectral_leading_eigenvalue"] = line.split("Leading eigenvalue:")[-1].strip()
        if "Eigenvalue gap (lead - MP_upper):" in line:
            # Strip trailing annotation like "  inside bulk"
        ...
        if "BANK VERDICT:" in line:
            data["verdict"] = line.split("BANK VERDICT:")[-1].strip()
...
def parse_no_key_output(path: str) -> dict:
    ...
    Returns dict with per-attempt results and summary.
    """
    data = {
        "attempts":   [],
        "n_blocked":  0,
    ...
    for line in lines:
        ...
        if line.startswith("[") and "]" in line and (
            "BLOCKED" in line or "TRIGGERED" in line
        ):
            ...
            if "BLOCKED" in rest:
                ...
                outcome = "TRIGGERED"
                data["n_triggered"] += 1
            data["attempts"].append({"name": name, "outcome": outcome})
...
def center(text: str, width=W) -> str:
    return text.center(width)

def rule(char="-", width=W) -> str:
    return char * width
...
def build_section_2(backdoor: dict) -> list:
    ...
    for app in backdoor["applicants"]:
        ...
        # Extract just the prediction labels for clean display
...
def build_section_3(no_key: dict) -> list:
    ...
    lines.append(center("8 Attack Attempts With File 1 Only (No File 2)"))
    lines.append(center(""))
    lines.append(sep())
    lines.append("")
    ...
    for i, attempt in enumerate(no_key["attempts"]):
        ...
        lines.append(f"  [{i+1}] {name:<48} {attempt['outcome']}")

    lines.append("  " + rule("-", W - 2))
    lines.append("")

    n_blocked   = no_key["n_blocked"]
    n_triggered = no_key["n_triggered"]
    n_total     = n_blocked + n_triggered

    lines.append(
        f"  Result: {n_blocked}/{n_total} attempts blocked, "
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate full side-by-side demo report."
    )
    parser.add_argument(
